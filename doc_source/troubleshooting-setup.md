...
## Mesh endpoint not accepting traffic on port 80<a name="mesh-endpoint-port-80"></a>

**Symptoms**  
Your mesh endpoint is not accepting traffic on port 80, but does accept traffic on other ports\. For example, you query the Envoy stats with the following command and receive a value other than zero\.
...
**Resolution**  
To resolve the issue, make sure that the Envoy is run as a user that can bind to port 80\. If you are using UID 1337, then either drop it so that the default user is used, or specify a user that can bind to port 80\.
