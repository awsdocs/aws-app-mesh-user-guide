...
Proxy authorization authorizes the [Envoy](envoy.md) proxy running within an Amazon ECS task, in a Kubernetes pod running on Amazon EKS, or running on an Amazon EC2 instance to read the configuration of one or more mesh endpoints from the App Mesh Envoy Management Service\. Proxy authorizaton will be enforced for all the new and upcoming features, which includes: [TLS functionality from ACM](https://docs.aws.amazon.com/app-mesh/latest/userguide/tls.html), and [Virtual Gateways](https://docs.aws.amazon.com/app-mesh/latest/userguide/virtual_gateways.html)\. It is recommended to enable this authorization to have a secure and consistent experience using IAM for authorization to specific resources\. An example of a configuration that the proxy can read is the private certificate of a mesh endpoints such as a virtual node or virtual gateway that has [Transport Layer Security \(TLS\)](tls.md) enabled\. Proxy authorization requires that the `appmesh:StreamAggregatedResources` permission is specified in an IAM policy\. The policy must be attached to an IAM role, and that IAM role must be attached to the compute resource that you host the proxy on\.
